...
| Batch | **Supported** | Create, retrieve, list, cancel, poll, upload input files, and download output/error files. | The extra YARA report helpers are local SDK features, not OpenAI endpoints. | [source](../src/api/batch/mod.rs) · [example](../examples/batch_processing_demo.rs) | [Batch API](https://developers.openai.com/api/reference/resources/batches/methods/create) |
