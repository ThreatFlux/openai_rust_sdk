...
curl https://api.openai.com/v1/models \
  ...
  -H "OpenAI-Organization: org-3QhVQi2SKSL0jwSa8GhyzsAP" \
...
Show properties
Returns
Returns 201 Created with the SDP answer in the response body. The Location response header includes the call ID for follow-up requests.

Example request
curl -X POST https://api.openai.com/v1/realtime/calls \
  -H "Authorization: Bearer $OPENAI_API_KEY" \
  -F "sdp=<offer.sdp;type=application/sdp" \
  -F 'session={"session":{"type":"realtime","model":"gpt-realtime"}};type=application/json'
Response
v=0
