...
impl BatchApi {
                                    ...
                                    if let Some(message) = first_choice.get("message") {
                                            ...
                                            message.get("content").and_then(|v| v.as_str())
                                        {
                                            if let Some(yara_rule) =
                                                YaraProcessor::extract_yara_rule(content)
                                            {
                                                let rule_filename = format!("{custom_id}.yar");
                                                let rule_path = output_dir.join(rule_filename);

                                                crate::helpers::write_string(&rule_path, yara_rule)
                                                    .await?;
    ...
    pub async fn generate_batch_report(
        ...
        if let Ok(content) = crate::helpers::read_string(results_file).await {
            for line in content.lines() {
                if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(line) {
                    if let Some(response) = parsed.get("response") {
                                ...
                                response.get("body").and_then(|b| b.get("choices"))
                            {
                                if let Some(first_choice) = choices.get(0) {
                                    if let Some(message) = first_choice.get("message") {
                                        ...
                                        {
                                            let has_yara =
                                                YaraProcessor::extract_yara_rule(content).is_some();
                                            report.add_successful_response(content.len(), has_yara);
                                        }
                                    }
                                }
                            }
        ...
        if let Some(errors_path) = errors_file {
            if let Ok(error_content) = crate::helpers::read_string(errors_path).await {
                for line in error_content.lines() {
                    if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(line) {
                        if let Some(error) = parsed.get("error") {
                            if let Some(code) = error.get("code").and_then(|v| v.as_str()) {
                                report.add_error_response(Some(code.to_string()));
                            }
                        }
                    }
                }
