...
fn create_user_validation_schema() -> Value {
    json!({
        "type": "object",
        "properties": {
...
fn test_validation_failure_cases(spec: &JsonSchemaSpec) {
...
fn create_validation_test_cases() -> Vec<(&'static str, Value)> {
    vec![
        (
...
fn test_valid_case(spec: &JsonSchemaSpec) {
    let valid_data = json!({
